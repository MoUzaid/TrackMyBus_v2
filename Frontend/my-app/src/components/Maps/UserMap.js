...
const UserMap = () => {
  ...
  useEffect(() => {
    const fetchBusData = async () => {
      ...
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/bus/${user.busId}`);
  ...
  const handleSendNotification = async () => {
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/send-multiple`, {
        userId,
        title: "Pickup Alert 🚍",
  ...
  const handleSendNotificationZero = async () => {
    try {
      const userId = user._id;
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/send-multiple`, {
        userId,
        title: "Pickup Alert 🚍",
        body: "Your Bus has been Arrived!"
      });
      console.log("Notification sent:", response.data);
    } catch (error) {
      console.error("Error sending notification:", error);
  ...
  useEffect(() => {
    pickupETAs.forEach((p) => {
      const minutes = parseInt(p.eta);
      if (minutes === 10 && !sentNotifications.name.includes(p.name)) {
        handleSendNotification();
        setSentNotifications(prev => [...prev, p.name]);
      }
      else if (minutes === 0 && sentNotifications.count < 1) {
        handleSendNotificationZero();
